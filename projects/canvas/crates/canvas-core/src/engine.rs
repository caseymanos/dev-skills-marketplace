...
impl Default for EngineOptions {
    fn default() -> Self {
        Self {
            preferred_backend: RenderBackend::WebGpu,
            device_pixel_ratio: 1.0,
            background_color: Color::new(0.2, 0.3, 0.5, 1.0), // Nice blue for testing
            debug: false,
            max_zoom: 256.0,
            min_zoom: 0.01,
        }
...
impl CanvasEngine {
    ...
    pub async fn init(&mut self, canvas: HtmlCanvasElement) -> Result<(), EngineError> {
        let width = canvas.width();
        let height = canvas.height();

        self.camera.set_viewport(width as f64, height as f64);

        let renderer = Renderer::new(canvas, width, height, self.options.background_color)
            .await
            .map_err(EngineError::RendererInit)?;
    ...
    pub fn render(&mut self) -> RenderStats {
        ...
        RenderStats {
            frame_time: start.elapsed().as_secs_f64() * 1000.0,
            ..stats
        }
    }
    ...
    pub fn handle_event(&mut self, event: InputEvent) -> bool {
        let consumed = self.tool_manager.handle_event(
            event,
            &mut self.world,
            &mut self.camera,
            &mut self.scene,
    ...
    pub fn zoom_to(&mut self, zoom: f64, center_x: f64, center_y: f64) {
        self.camera.zoom_to(zoom, center_x, center_y);
        self.needs_render = true;
    }

    pub fn screen_to_canvas(&self, screen_x: f64, screen_y: f64) -> Point {
        self.camera.screen_to_canvas(screen_x, screen_y)
    }

    pub fn canvas_to_screen(&self, canvas_x: f64, canvas_y: f64) -> Point {
        self.camera.canvas_to_screen(canvas_x, canvas_y)
    }
    ...
    pub fn set_selection(&mut self, ids: Vec<ObjectId>) {
        self.selection.selected_ids = ids;
        self.needs_render = true;
    }
